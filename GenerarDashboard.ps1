...
# Esto permite actualizar el script desde ialopez sin TeamViewer
# =============================================
$scriptOneDrive = "$carpetaUser\GenerarDashboard.ps1"
$scriptLocal    = "$carpetaACTUM\GenerarDashboard.ps1"
if ((Test-Path $scriptOneDrive) -and (Test-Path $scriptLocal)) {
    ...
    if ($tsOneDrive -gt $tsLocal.AddSeconds(10)) {
        Write-Host "Auto-update: version nueva detectada en OneDrive. Actualizando..." -ForegroundColor Cyan
...
# Sin ningun caracter especial en el codigo fuente - funciona en cualquier sistema
# =============================================
function ConvertTo-HtmlEntities {
    param([string]$text)
    if ([string]::IsNullOrEmpty($text)) { return $text }
    # Minusculas acentuadas
    $text = $text -replace [char]0xE1, '&aacute;'   # a
    $text = $text -replace [char]0xE9, '&eacute;'   # e
    $text = $text -replace [char]0xED, '&iacute;'   # i
    $text = $text -replace [char]0xF3, '&oacute;'   # o
    $text = $text -replace [char]0xFA, '&uacute;'   # u
    $text = $text -replace [char]0xF1, '&ntilde;'   # n tilde
    $text = $text -replace [char]0xFC, '&uuml;'     # u dieresis
    # Mayusculas acentuadas
    $text = $text -replace [char]0xC1, '&Aacute;'   # A
    $text = $text -replace [char]0xC9, '&Eacute;'   # E
    $text = $text -replace [char]0xCD, '&Iacute;'   # I
    $text = $text -replace [char]0xD3, '&Oacute;'   # O
    $text = $text -replace [char]0xDA, '&Uacute;'   # U
    $text = $text -replace [char]0xD1, '&Ntilde;'   # N tilde
    $text = $text -replace [char]0xDC, '&Uuml;'     # U dieresis
    return $text
}
...
try {
    ...
    # Intento 1: JOIN Consigna+Caja (clave = Consigna.CodigoCliente = lo que va en el CSV)
    ...
    # Intento 2: si el JOIN con Activa=1 no devolvio nada, usar SELECT directo de Caja
    # (puede pasar si la columna Activa no existe o no hay consignas activas marcadas)
    if ($mapaCajas.Count -eq 0) {
        Write-Host "JOIN Consigna+Caja devolvio 0 filas, usando SELECT FROM Caja directo..." -ForegroundColor Yellow
        $cmd2 = $conn.CreateCommand()
        $cmd2.CommandText = "SELECT Codigo, CodigoCliente, Descripcion, FechaCaducidad FROM Caja"
        $reader2 = $cmd2.ExecuteReader()
        while ($reader2.Read()) {
            $key = "$([int]$reader2['Codigo'])"
            $fechaCad = if ($reader2['FechaCaducidad'] -ne [DBNull]::Value) { [DateTime]$reader2['FechaCaducidad'] } else { $null }
            $mapaCajas[$key] = @{
                CodigoCliente  = "$($reader2['CodigoCliente'])"
                Descripcion    = "$($reader2['Descripcion'])"
                FechaCaducidad = $fechaCad
            }
        }
        $reader2.Close()
    ...
    $conn.Close()
    Write-Host "SQL OK: $($mapaCajas.Count) instrumentos cargados desde BD" -ForegroundColor Green
} catch {
    Write-Host "SQL no disponible ($($_.Exception.Message)), usando EXPORT_Cajas.txt..." -ForegroundColor Yellow
    ...
    if (Test-Path $exportCajasPath) {
        ...
        Write-Host "Fallback EXPORT_Cajas.txt: $($mapaCajas.Count) instrumentos cargados" -ForegroundColor Yellow
    } else {
        Write-Host "EXPORT_Cajas.txt no encontrado en $exportCajasPath" -ForegroundColor Red
    }
}
...
if ($historial.Count -gt 0) {
    ...
    foreach ($mov in $ultimaAccionPorConsigna) {
        ...
        $estadoInstrumentos += [PSCustomObject]@{
            ...
            Instrumento       = ConvertTo-HtmlEntities $cajaInfo.Descripcion
...
foreach ($instrumento in ($estadoInstrumentos | Sort-Object { [int]$_.NumConsigna })) {
...
# CONVERSION FINAL: convertir TODOS los caracteres no-ASCII a entidades HTML numericas
# Esto es la red de seguridad definitiva - aunque algun char escape a ConvertTo-HtmlEntities,
# aqui se convierte antes de escribir el fichero.
# El HTML resultante es ASCII puro = imposible que haya problemas de encoding.
$htmlFinal = [System.Text.RegularExpressions.Regex]::Replace($html, '[^\x00-\x7F]', {
    param($match)
    return '&#' + [int][char]$match.Value + ';'
})
# WriteAllText con ASCII encoding: HTML entities garantizan tildes correctas
# OneDrive detecta el cambio porque el proceso OneDrive.exe esta corriendo
...
Write-Host "Instrumentos cargados: $($mapaCajas.Count) | Movimientos: $totalMovimientos" -ForegroundColor Cyan
